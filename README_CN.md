...
- 最近完成的工作重点在 `Block` / `BlockState` / `Item` / `ItemStack` / `Entity` 扩展桥，以及 `EventHooks` 覆盖补全。
- 当前最大缺口仍然是 `CommonHooks`、客户端渲染钩子、复杂实体同步链，以及 NeoForge 对原版类的深层补丁行为。

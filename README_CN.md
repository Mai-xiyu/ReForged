...
| **Mixin 系统** | 46 个补丁用于 Minecraft/Forge 兼容性 |
...
| **大型内容模组** | Mekanism、Create 等 | ⚠️ 部分 | 已实现 DataMap、BiomeModifier 编解码、属性修改器事件、RenderBuffers 注入、DimensionSpecialEffects 注入、GUI 层级排序等；大部分核心功能可运行，少量边缘路径可能需要额外补丁 |
...
- 共 782 个 Java 源文件（688 个 shim + 94 个核心），46 个 Mixin 补丁，60 个事件 wrapper 构造函数。
